...
draw.PM <- function(x, uncert = .12) {
  x |>
    mutate(
      concentration = map_chr(
        as.numeric(concentration),
        ~ rnorm(1, .x, .x * uncert / 1.96) |> matchable(1)
      )
    )
}
