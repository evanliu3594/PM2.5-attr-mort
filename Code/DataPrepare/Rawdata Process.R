...
st_as_stars(st_bbox(), dx = .5, dy = .5) |>
  st_join(world, as_points = F) |>
  as.data.frame() |>
  filter(!is.na(iso_a3)) |>
  select(x, y, Country = iso_a3) |>
  write_xlsx(str_glue(
    './Data/GRID_information_instance_{format(Sys.Date(), "%y%m%d")}_iso.xlsx'
  ))
  ...
  map_dfr(
    ~ {
      read_ncdf(.x) |>
        ...
        na.omit() |>
        mutate(
          across(lon:lat, \(x) round(x, 2) |> as.character),
...
pm25 |>
  rename(x = lon, y = lat) |>
  write_xlsx(str_glue(
    './Data/GridPM25_instance_{format(Sys.Date(), "%y%m%d")}.xlsx'
  ))
...
unzipfiles <- str_subset(
  list.files(
    file.path(
      Sys.getenv("NJUBOX"),
      "./0.Data/5.PopData/LandScan_PopDistribution"
    ),
    pattern = ".zip",
    full.names = T
  ),
...
pop <- list.files(
  ...
  map_dfr(
    ~ {
      read_stars(.x) |>
        st_warp(
          st_as_stars(st_bbox(), values = NA, dx = .5, dy = .5),
          use_gdal = T,
          method = 'sum'
        ) |>
        `names<-`('pop') |>
...
pop |>
  set_names(names(pm25)) |>
  rename(x = lon, y = lat) |>
  write_xlsx(str_glue(
    './Data/GridPop_instance_{format(Sys.Date(), "%y%m%d")}.xlsx'
  ))

file.path("D:/temp/", unzipfiles) |> file.remove()

mortrate <- './Data/IHME_MortRate_raw/IHME-GBD_2019_DATA-96c44824-1.csv' |>
  read_csv()
...
mortrate |>
  ...
  write_xlsx(str_glue(
    './Data/GBD_mortality_instance_{format(Sys.Date(), "%y%m%d")}.xlsx'
  ))
...
agegroup |>
  ...
  pivot_wider(
    names_from = 'year',
    values_from = 'agestruc',
    values_fn = mean
  ) |>
  rename(base2015 = `2015`, `SSP1-Baseline_2030` = `2019`) |>
  write_xlsx(str_glue(
    './Data/GBD_agestructure_instance_{format(Sys.Date(), "%y%m%d")}.xlsx'
  ))
