...
normalize_coords <- function(df) {
  ...
  lon_candidates <- c("longitude", "Longitude", "LONGITUDE",
                       "long", "Long", "LONG",
                       "lon", "Lon", "LON",
                       "x", "X")

  # Latitude candidates
  lat_candidates <- c("latitude", "Latitude", "LATITUDE",
                       "lat", "Lat", "LAT",
                       "y", "Y")
  ...
  if (is.na(lon_col) || is.na(lat_col)) {
    ...
    stop(
      ...
      if (length(found_lon) > 0) paste(found_lon, collapse = ", ") else "(none)", "\n",
      "  Found latitude-like columns:  ",
      if (length(found_lat) > 0) paste(found_lat, collapse = ", ") else "(none)", "\n",
  ...
  if (lon_col != "x" || lat_col != "y") {
    ...
    cat(str_glue("  Coordinates normalized: \"{lon_col}\" -> \"x\", \"{lat_col}\" -> \"y\"\n"))
  }
...
read_files <- function(Grids, Pop, Conc_real, Conc_cf, MortRate, AgeGroup,
  ...
  for (f in list(Grids = Grids, Pop = Pop, Conc_real = Conc_real, MortRate = MortRate, AgeGroup = AgeGroup)) {
    if (!file.exists(f))
      stop("File not found: \"", f, "\". Check that the path is correct and the file exists.")
  }
  ...
  # ---- Guard: Grid_info required columns ----
  if (!all(c("x", "y") %in% names(Grid_info)))
    stop("Grid_info must contain geographic coordinate columns (x/lon/long, y/lat). ",
         "Found: ", paste(names(Grid_info), collapse = ", "))
  ...
  if (!all(c("x", "y") %in% names(Conc_real)))
    stop("Conc_real must contain geographic coordinate columns (x/lon/long, y/lat). ",
         "Found: ", paste(names(Conc_real), collapse = ", "))

  # Specify UNREAL PM2.5 data, used for only counter-fact scenario.

  if (file.exists(Conc_cf)) {
    conc_cf_data <- fuse_read(Conc_cf) %>%
      normalize_coords() %>%
      mutate(
        across(where(is.numeric) & x:y, ~ matchable(.x, dgt = dgt_grid)),
        across(where(is.numeric) & -x:-y, ~ matchable(.x, dgt = dgt_conc))
      )
    if (!all(c("x", "y") %in% names(conc_cf_data)))
      stop("Conc_cf must contain geographic coordinate columns (x/lon/long, y/lat). ",
           "Found: ", paste(names(conc_cf_data), collapse = ", "))
  } else {
    conc_cf_data <- NULL
  }

  assign('Conc_cf', envir = globalenv(), conc_cf_data)

  if (is.null(Conc_cf))
    warning("Conc_cf file \"", Conc_cf, "\" not found — Conc_cf set to NULL. ",
  ...
  if (!all(c("domain", "endpoint", "agegroup") %in% names(mort_rate_raw)))
    stop("MortRate must contain columns \"domain\", \"endpoint\", \"agegroup\". Found: ",
  ...
  if (!all(c("domain", "agegroup") %in% names(age_group_raw)))
    stop("AgeGroup must contain columns \"domain\", \"agegroup\". Found: ",
  ...
  if (!exists('.CR_Model', envir = globalenv())) {
    stop("C-R model not set. Call set_Model() before read_files(). ",
  ...
  if (length(missing_sheets) > 0)
    warning("RR lookup table is missing expected sheet(s): ",
  ...
  n_pop   <- nrow(Pop)
  overlap_gc <- Grid_info %>% select(x, y) %>% inner_join(Conc_real %>% select(x, y), by = c("x", "y")) %>% nrow
  overlap_gp <- Grid_info %>% select(x, y) %>% inner_join(Pop %>% select(x, y), by = c("x", "y")) %>% nrow
  ...
  if (overlap_gc < n_grid * 0.8 || overlap_gp < n_grid * 0.8)
    warning("Low coordinate overlap between Grid_info and Conc/Pop data. ",
...
RR_std <- function(RR_index = "MEAN") {
  ...
  if (!is.list(RR_table))
    stop("RR_table is not a list (found: ", class(RR_table),
  ...
  if (!RR_index %in% names(RR_table))
    stop("RR_index \"", RR_index, "\" not found in RR_table. ",
         "Available indices: ", paste(names(RR_table), collapse = ", "))
  ...
  if (nrow(RR_tbl) == 0)
    stop("RR_table[[\"", RR_index, "\"]] is empty after pivot_longer. ",
  ...
  if (nrow(RR_reshape) == 0)
    stop("RR_reshape is empty for model \"", CR, "\" with index \"", RR_index,
  ...
  if (nrow(na_rr) > 0)
    warning(nrow(na_rr), " NA values remain in RR_reshape after fill. ",
            ...
            "First few: concentration=", paste(head(unique(na_rr$concentration), 3), collapse = ", "))
...
getMortRate <- function(at) {
  ...
  if (!at %in% names(MortRate))
    stop("Year/scenario \"", at, "\" not found in MortRate columns. ",
         "Available: ", paste(setdiff(names(MortRate), c("domain", "endpoint", "agegroup")), collapse = ", "))
...
getConc_real <- function(at) {
  ...
  if (!at %in% names(Conc_real))
    stop("Year/scenario \"", at, "\" not found in Conc_real columns. ",
         "Available: ", paste(setdiff(names(Conc_real), c("x", "y")), collapse = ", "))
...
getConc_cf <- function(at) {
  if (!exists('Conc_cf', envir = globalenv()) || is.null(Conc_cf))
    stop("Conc_cf not found in global environment (it was NULL or not loaded). ",
         "Ensure the Conc_cf file exists and was loaded by read_files().")
  if (!at %in% names(Conc_cf))
    stop("Year/scenario \"", at, "\" not found in Conc_cf columns. ",
         "Available: ", paste(setdiff(names(Conc_cf), c("x", "y")), collapse = ", "))
...
getPop <- function(at) {
  ...
  if (!at %in% names(Pop))
    stop("Year/scenario \"", at, "\" not found in Pop columns. ",
         "Available: ", paste(setdiff(names(Pop), c("x", "y")), collapse = ", "))
...
getAgeGroup <- function(at) {
  ...
  if (!at %in% names(AgeGroup))
    stop("Year/scenario \"", at, "\" not found in AgeGroup columns. ",
         "Available: ", paste(setdiff(names(AgeGroup), c("domain", "agegroup")), collapse = ", "))
...
Mortality <- function(Grids, Conc_r, Conc_c = NULL, ag, mRate, pop, RR, domain = NULL) {
  ...
  if (!all(domain %in% names(Grids)))
    stop("Domain column \"", paste(setdiff(domain, names(Grids)), collapse = "\", \""),
         ...
         "Available columns: ", paste(names(Grids), collapse = ", "))

  RR_tbl <- RR_std(RR)

  # ---- Guard: RR lookup table not empty ----
  if (nrow(RR_tbl) == 0)
    stop("RR lookup table is empty for index \"", RR,
         "\". Check that RR_table[[\"", RR, "\"]] exists and contains data.")
  ...
  if (any(is.na(PWRR$PWRR)) || any(is.infinite(PWRR$PWRR)) || any(PWRR$PWRR <= 1))
    stop("Invalid PWRR detected. ",
         ...
         if (any(PWRR$PWRR <= 1)) "Some domains have PWRR <= 1 (population-weighted mean RR ",
  ...
  if (n_after_mort == 0)
    stop("Mortality step: all ", n_before_mort, " rows dropped by na.omit. ",
  ...
  if (n_after_mort < n_before_mort * 0.5)
    warning("Mortality step: ", n_before_mort - n_after_mort, " of ", n_before_mort,
            " rows (", round(100 * (1 - n_after_mort / n_before_mort)), "%) dropped by na.omit.")
  ...
  if (nrow(result) == 0)
    stop("Mortality result has 0 rows. ",
...
Uncertainty <- function(PWE, aggr_pop, age_struc, m_Rate, 
  ...
  left_join(
    Sensi_up %>% group_by(domain) %>%
      summarise(CI_UP = sqrt(sum(Sensi ^ 2)), .groups = "drop"),
    Sensi_low %>% group_by(domain) %>%
